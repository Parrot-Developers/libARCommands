...
for cl in allClassesNames:
    ...
    for cmd in cmdList:
        ...
        for argN in ANList:
            ...
            argT = ATList [aIndex]
            if 'string' == argT:
                cfile.write ('                    if (' + argN + ' != NULL) { free (' + argN + '); }\n')
        cfile.write ('                }\n')
        cfile.write ('                else\n')
        cfile.write ('                {\n')
...
jfile.write ('\n')
jfile.write ('    /**\n')
jfile.write ('     * Creates a new, empty ' + JNIClassName + '.<br>\n')
jfile.write ('     * This is the typical constructor for app-generated ' + JNIClassName + '.<br>\n')
...
jfile.write ('     * This is the typical constructor for network-generated ' + JNIClassName + '.<br>\n')
jfile.write ('     * To optimize memory, the application can reuse an ' + JNIClassName + '\n')
jfile.write ('     * object after it was disposed.\n')
jfile.write ('     */\n')
...
jfile.write ('        pdata = 0;\n')
jfile.write ('        dataSize = 0;\n')
jfile.write ('        valid = nativeCopyFromArray (oldData);\n')
...
jfile.write ('\n')
jfile.write ('    /**\n')
jfile.write ('     * Tells the native part that it can free the memory.<br>\n')
jfile.write ('     * Calling <code>dispose ()</code> on an ' + JNIClassName + ' mark it as invalid, and\n')
jfile.write ('     * free any memory allocated from C.<br>\n')
jfile.write ('     * This is the only way to avoid leaking the C memory.\n')
jfile.write ('     */\n')
jfile.write ('    public void dispose () {\n')
jfile.write ('        if (pdata != 0) {\n')
jfile.write ('            nativeFreeData (pdata, createdFromArray);\n')
jfile.write ('            pdata = 0;\n')
jfile.write ('            dataSize = 0;\n')
jfile.write ('            valid = false;\n')
jfile.write ('            createdFromArray = false;\n')
...
jfile.write ('     * @return The native data pointer for the ' + JNIClassName + ', or <code>0</code> if the ' + JNIClassName + ' is not valid\n')
jfile.write ('     */\n')
...
jfile.write ('     * This function is specified by <code>ARNativeData</code> interface\n')
jfile.write ('     * @return The native size of the ' + JNIClassName + ', or <code>0</code> if the ' + JNIClassName + ' is not valid\n')
jfile.write ('     */\n')
...
jfile.write ('     * This function is specified by <code>ARNativeData</code> interface\n')
jfile.write ('     * @return A byte-equivalent copy of the native data, or <code>null</code> if the ' + JNIClassName + ' is not valid\n')
jfile.write ('     */\n')
jfile.write ('    public byte [] getByteData () {\n')
jfile.write ('        if (valid == false) {;\n')
...
jfile.write ('     * This function <code>dispose</code> the current ' + JNIClassName + ', then replace it with the\n')
jfile.write ('     * byte array content, typically retreived from a network stream\n')
jfile.write ('     * @return <code>true</code> if the command was properly set, <code>false</code> if a copy error occured\n')
jfile.write ('     */\n')
...
for cl in allClassesNames:
    ...
    for cmd in cmdList:
        ...
        jfile.write ('     * <br>\n')
        jfile.write ('     * This function <code>dispose</code> the current ' + JNIClassName + ', then replace it with a\n')
        jfile.write ('     * new command created from the current params\n')
        for argN in ANList:
            aIndex = ANList.index (argN)
            for argC in ACList [aIndex]:
                jfile.write ('     * @param ' + argN + ' ' + argC + '\n')
        jfile.write ('     * @return <code>true</code> if the command was properly set, <code>false</code> otherwise\n')
        jfile.write ('     */\n')
        ...
        jfile.write (') {\n')
        jfile.write ('        dispose ();\n')
        jfile.write ('        valid = nativeSet'+ ARCapitalize (cl) + ARCapitalize (cmd) + ' (')
        first = 1
        for argN in ANList:
            if 1 == first:
                first = 0
...
jfile.write ('    private native boolean nativeCopyFromArray (byte [] oldData);\n')
jfile.write ('    private native void    nativeFreeData (long dataToFree, boolean wasCreatedFromArray);\n')
...
for cl in allClassesNames:
    ...
    cmdList = commandsNameByClass[cIndex]
    for cmd in cmdList:
        jfile.write ('    private native boolean nativeSet'+ ARCapitalize (cl) + ARCapitalize (cmd) + ' (')
        cmIndex = cmdList.index (cmd)
        ANList = argNamesByClassAndCommand [cIndex][cmIndex]
        ATList = argTypesByClassAndCommand [cIndex][cmIndex]
        first = 1
...
cfile.write ('#include <stdlib.h>\n')
cfile.write ('\n')
cfile.write ('static jfieldID g_pdata_id = 0;\n')
cfile.write ('static jfieldID g_dataSize_id = 0;\n')
cfile.write ('static JavaVM *g_vm = NULL;\n')
cfile.write ('\n')
cfile.write ('JNIEXPORT jboolean JNICALL\n')
cfile.write (JNI_FUNC_PREFIX+ JNIClassName+ '_nativeCopyFromArray ('+ JNI_FIRST_ARGS+ ', jbyteArray oldData)\n')
cfile.write ('{\n')
cfile.write ('    jboolean valid = JNI_TRUE;\n')
cfile.write ('\n')
cfile.write ('    if ((g_pdata_id == 0) &&\n')
cfile.write ('        (g_dataSize_id == 0))\n')
cfile.write ('    {\n')
cfile.write ('        jclass clz = (*env)->GetObjectClass (env, thizz);\n')
cfile.write ('        if (clz != 0)\n')
cfile.write ('        {\n')
cfile.write ('            g_pdata_id = (*env)->GetFieldID (env, clz, "pdata", "J");\n')
...
cfile.write ('    }\n')
cfile.write ('\n')
cfile.write ('    if ((valid == JNI_FALSE) &&\n')
cfile.write ('        (c_pdata != NULL))\n')
cfile.write ('    {\n')
...
cfile.write ('    }\n')
cfile.write ('\n')
cfile.write ('    (*env)->SetLongField (env, thizz, g_pdata_id, (jlong) (intptr_t)c_pdata);\n')
cfile.write ('    (*env)->SetIntField (env, thizz, g_dataSize_id, (jint)len);\n')
cfile.write ('\n')
cfile.write ('    return valid;\n')
cfile.write ('}\n')
cfile.write ('\n')
cfile.write ('JNIEXPORT void JNICALL\n')
cfile.write (JNI_FUNC_PREFIX+ JNIClassName+ '_nativeFreeData ('+ JNI_FIRST_ARGS+ ', jlong dataToFree, jboolean jcreatedFromArray)\n')
cfile.write ('{\n')
cfile.write ('    uint8_t *c_pdata = (uint8_t *) (intptr_t)dataToFree;\n')
cfile.write ('    if (jcreatedFromArray == JNI_FALSE)\n')
cfile.write ('    {\n')
...
for cl in allClassesNames:
    ...
    for cmd in cmdList:
        ...
        ATList = argTypesByClassAndCommand [cIndex][cmIndex]
        cfile.write ('JNIEXPORT jboolean JNICALL\n')
        cfile.write (JNI_FUNC_PREFIX+ JNIClassName+ '_nativeSet'+ ARCapitalize (cl) + ARCapitalize (cmd) + ' ('+ JNI_FIRST_ARGS)
        for argN in ANList:
            aIndex = ANList.index (argN)
            argT = ATList [aIndex]
            jargT = xmlToJni (argT)
