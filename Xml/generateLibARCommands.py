...
#Name of the output C/H testbench file
TB_CFILE_NAME='autoTest.c'
TB_HFILE_NAME='autoTest.h'
TB_EP_CFILE_NAME='autoTest_ep.c'
...
TB_HFILE=TB__DIR+TB_HFILE_NAME
GENERATED_FILES.append (TB_HFILE)
TB_EP_CFILE=TB__DIR+TB_EP_CFILE_NAME
GENERATED_FILES.append (TB_EP_CFILE)
...
for cl in allClassesNames:
    ...
    for cmd in cmdList:
        ...
        hfile.write (' */\n')
        hfile.write ('uint8_t *libARCommandsGenerate' + cl.capitalize() + cmd.capitalize() + ' (int32_t *buffLen')
        for argN in ANList:
...
for cl in allClassesNames:
    ...
    for cmd in cmdList:
        cmdIndex = cmdList.index (cmd)
        cfile.write ('uint8_t *libARCommandsGenerate' +  cl.capitalize() + cmd.capitalize() + ' (int32_t *buffLen')
        ATList = argTypesByClassAndCommand [clIndex][cmdIndex]
        ANList = argNamesByClassAndCommand [clIndex][cmdIndex]
        for argN in ANList:
            argT = ATList [ANList.index (argN)]
        ...
        cfile.write ('    currBufferSize = LIBARCOMMANDS_GEN_BUFFER_SIZE;\n')
...
for cl in allClassesNames:
    ...
    for cmd in cmdList:
        cmdIndex = cmdList.index (cmd)
        cfile.write ('    res = libARCommandsGenerate' + cl.capitalize() + cmd.capitalize() + ' (&resSize')
        ATList = argTypesByClassAndCommand [clIndex][cmdIndex]
        ANList = argNamesByClassAndCommand [clIndex][cmdIndex]
        for argN in ANList:
            argT = ATList [ANList.index (argN)]
